...
def demo3():
    ...
    washing3()
    '''
    尝试运行一下, 我们会发现还是会报错 (报错内容和 demo2 一样). 
    原因:
        1.  await 后面必须跟一个 awaitable 类型或者具有 __await__ 属性的
        对象. 这个 awaitable, 并不是我们认为 sleep() 是 awaitable 就可以 await 了,
        await asyncio.sleep(3)  asyncio 库的 sleep() 机制与 time.sleep() 不同, 
        前者是 "假性睡眠", 后者是会导致线程阻塞的 "真性睡眠"
        2.  如果我们要执行异步函数, 不能用这样的调用方法:
            washing1()
            washing2()
            washing3() 而应该用 asyncio 库中的事件循环机制来启动
    以下是不可等待的:
            await time.sleep(3)
